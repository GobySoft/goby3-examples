...
PluggableUDPMulticastDriver::PluggableUDPMulticastDriver() = default;
PluggableUDPMulticastDriver::~PluggableUDPMulticastDriver() = default;

const char* goby_driver_name() { return "udp_multicast_driver"; }
goby::acomms::ModemDriverBase* goby_make_driver() { return new PluggableUDPMulticastDriver(); }

void PluggableUDPMulticastDriver::startup(const goby::acomms::protobuf::DriverConfig& cfg)
{
    driver_cfg_ = cfg;
...
void PluggableUDPMulticastDriver::shutdown()
{
    io_context_.stop();
    socket_.close();
}

void PluggableUDPMulticastDriver::handle_initiate_transmission(
...
void PluggableUDPMulticastDriver::do_work() { io_context_.poll(); }

void PluggableUDPMulticastDriver::receive_message(
    const goby::acomms::protobuf::ModemTransmission& msg)
{
    if (msg.type() == goby::acomms::protobuf::ModemTransmission::DATA && msg.ack_requested() &&
...
void PluggableUDPMulticastDriver::start_send(const goby::acomms::protobuf::ModemTransmission& msg)
{
    // send the message
    std::string bytes;
...
void PluggableUDPMulticastDriver::send_complete(
...
void PluggableUDPMulticastDriver::start_receive()
{
...
void PluggableUDPMulticastDriver::receive_complete(
    const boost::system::error_code& error, std::size_t bytes_transferred)
{
    if (error)
    {
