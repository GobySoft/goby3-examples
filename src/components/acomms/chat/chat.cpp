...
int main(int argc, char* argv[])
{
    ...
    goby::acomms::bind(mm_driver_, q_manager_, mac_);
...
#ifdef USE_FLEXIBLE_DATA_PACKET
    driver_cfg.MutableExtension(goby::acomms::micromodem::protobuf::Config)
        ->add_nvram_cfg("psk.packet.mod_hdr_version,1");
#endif
...
void monitor_modem_receive(const goby::acomms::protobuf::ModemTransmission& rx_msg)
{
    if (rx_msg.GetExtension(goby::acomms::micromodem::protobuf::transmission).type() ==
            goby::acomms::micromodem::protobuf::MICROMODEM_TWO_WAY_PING &&
        rx_msg.GetExtension(goby::acomms::micromodem::protobuf::transmission).has_ranging_reply())
    {
        const auto& range_reply =
            rx_msg.GetExtension(goby::acomms::micromodem::protobuf::transmission).ranging_reply();
