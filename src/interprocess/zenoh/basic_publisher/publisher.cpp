...
#include <goby/zenoh/application/single_thread.h> // provides SingleThreadApplication
...
using Base = goby::zenoh::SingleThreadApplication<BasicPublisherConfig>;
...
class BasicPublisher : public Base
{
  public:
    ...
    // virtual method in goby::zenoh::SingleThreadApplication called at the frequency given to Base(freq)
    void loop() override
    {
...
// these can be set on the command line (try "basic_publisher_zenoh --help" to see parameters)
// or in a configuration file (use "basic_publisher_zenoh --example_config" for the correct syntax)
