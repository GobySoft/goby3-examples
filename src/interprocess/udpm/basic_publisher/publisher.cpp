...
#include <goby/udpm/application/single_thread.h> // provides SingleThreadApplication
...
using Base = goby::udpm::SingleThreadApplication<BasicPublisherConfig>;
...
class BasicPublisher : public Base
{
  public:
    ...
    // virtual method in goby::udpm::SingleThreadApplication called at the frequency given to Base(freq)
    void loop() override
    {
...
// these can be set on the command line (try "basic_publisher_udpm --help" to see parameters)
// or in a configuration file (use "basic_publisher_udpm --example_config" for the correct syntax)
